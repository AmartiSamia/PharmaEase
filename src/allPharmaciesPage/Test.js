...
const Test = ({ navigation }) => {
  ...
  const fetchSearchResults = (query) => {
    return pharmacies.filter((result) =>
      result.pharmacy.toLowerCase().includes(query.toLowerCase())
    );
  };
  ...
  const renderItem = ({ item }) => {
    const isEmergency = pharmaciesData.some(
      (emergencyPharmacy) => emergencyPharmacy.name === item.pharmacy
    );
